...
DOCUMENTATION = r'''module: postgresql_idx
...
options:
  ...
  state:
    ...
    - C(stat) returns index statistics information from the ``pg_stat_user_indexes``
      standard view. Supported from Ansible 2.10.
    - 'When C(stat) following parameters will be ignored:'
    - I(schema), I(table), I(columns), I(cond), I(idxtype), I(tablespace), I(concurrent),
      I(cascade).
    type: str
    default: present
    choices:
  ...
  concurrent:
    ...
    - Pay attention, if I(concurrent=no), the table will be locked (ACCESS EXCLUSIVE)
      during the building process. For more information about the lock levels see
      U(https://www.postgresql.org/docs/current/explicit-locking.html).
    - If the building process was interrupted for any reason when I(cuncurrent=yes),
      the index becomes invalid. In this case it should be dropped and created again.
