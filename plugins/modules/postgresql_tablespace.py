...
DOCUMENTATION = r'''module: postgresql_tablespace
...
options:
  ...
  owner:
    ...
    - If this option is not specified, the tablespace owner is a role that creates
      the tablespace.
    type: str
  set:
    description:
    ...
    - When reset is passed as an option's value, if the option was set previously,
      it will be removed.
    type: dict
  rename_to:
    description:
  ...
  db:
    ...
    - login_db
notes:
- I(state=absent) and I(state=present) (the second one if the tablespace doesn't exist)
  do not support check mode because the corresponding PostgreSQL DROP and CREATE TABLESPACE
  commands can not be run inside the transaction block.
seealso:
