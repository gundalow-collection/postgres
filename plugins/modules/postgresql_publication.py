...
DOCUMENTATION = r'''module: postgresql_publication
...
options:
  ...
  tables:
    ...
    - If the publication already exists for specific tables and I(tables) is not passed,
      nothing will be changed. If you need to add all tables to the publication with
      the same name, drop existent and create new without passing I(tables).
    type: list
    elements: str
  state:
    description:
